...
export const bind = (store) => {
  ...
  poll = async (force) => {
...

    {
      const byIndexAssets = await Promise.all(schemas.map(async s => {
        const assets = await Promise.all(s.functions.assetsOfOwnerByIndex.map(async assetsOfOwnerByIndex => {
          ...
          const func = async (index, addr, proxy, array) => {
            ...
            if (asset !== null) {
              var owner
              if (s.functions.ownerOf) {
                const ownerOf = s.functions.ownerOf(asset)
                const contract = web3.eth.contract([ownerOf]).at(ownerOf.target)
      ...
      if (store.state.web3.assets === undefined) {
        store.commit('setWeb3Assets', [].concat(...byIndexAssets))
      }
